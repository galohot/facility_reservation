        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="flex justify-between mt-4 col-6">
                    <form action="{{ secure_url(route('addons.index') }}" method="GET" class="flex items-center">
                    ...
                    <a href="{{ secure_url(route('addons.create') }}" class="m-2 btn btn-success">
                        Create {{$pageTitle}}
                      </a>
                  </div>
              <div id="table-default" class="table-responsive">
                <table class="table">
                  ...
                  <tbody class="table-tbody">
                    ...
                    <tr>
                      <td>{{ $addon->id }}</td>
                      <td>{{ $addon->addon_str }}</td>
                      <td>{{ $addon->description }}</td>
                      <td>
                        <div class="mr-2" role="group" aria-label="User Actions">
                            <div class="m-1 d-block">
                                <a href="{{ secure_url(route('addons.show', $addon->id) }}" class="btn btn-primary" role="button">
                                  <i class="fas fa-eye"></i> View
                                </a>
                            </div>
                            <div class="m-1 d-block">
                                <a href="{{ secure_url(route('addons.edit', $addon->id) }}" class="btn btn-secondary" role="button">
                                  <i class="fas fa-pencil-alt"></i> Edit
                                </a>
                            </div>
                            <form action="{{ secure_url(route('addons.destroy', $addon->id) }}" method="POST" class="inline">
                              @csrf
                              @method('DELETE')
                              <div class="m-1 d-block">
