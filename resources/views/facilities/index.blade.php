                    ...
                    <form action="{{ route('facilities.index') }}" method="GET" class="flex items-center">
                        ...
                        <select name="category" class="m-2 form-control d-inline-flex"> <!-- New select input for category filter -->
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->category_str }}" {{ request('category') == $category->category_str ? 'selected' : '' }}>{{ $category->category_str }} (jumlah fasilitas: {{ $category->facilities->count() }})</option>
                            @endforeach
                        </select>
                    ...
                    <table class="table">
                        ...
                        <tbody class="table-tbody">
                            ...
                            <tr>
                                ...
                                <td class="sort-name"><a href="{{ route('facilities.show', $facility->id) }}">{{ $facility->name }}</a></td>
                                ...
                                <td class="sort-facilitymanager">{{ $facility->ukerMaster->nama_unit_kerja_eselon_2 }}, {{ $facility->ukerMaster->satkerMaster->nama_satker }}</td>
                                <td class="sort-actions">
                                    <div class="mr-2" role="group" aria-label="User Actions">
                                        <div class="m-1 d-block">
                                            <a href="{{ route('facilities.show', $facility->id) }}" class="btn btn-primary" role="button">
                                                <i class="fas fa-eye"></i> View
                                            </a>
                                        </div>
                                        @if(auth()->check() && (auth()->user()->hasRole('admin') || auth()->user()->hasRole('manager')))
                                        <div class="m-1 d-block">
                                            <a href="{{ route('facilities.edit', $facility->id) }}" class="btn btn-secondary" role="button">
